# P T2AV-Compass: Towards Unified Evaluation for Text-to-Audio-Video Generation

Zhe Cao \( {}^{1 * } \) , Tao Wang \( {}^{1 * } \) , Jiaming Wang \( {}^{1 * } \) , Yanghai Wang \( {}^{1 * } \) , Yuanxing Zhang \( {}^{2} \) , Jialu Chen \( {}^{2} \) , Miao Deng \( {}^{1} \) , Jiahao Wang \( {}^{1} \) , Yubin Guo \( {}^{1} \) , Chenxi Liao \( {}^{1} \) , Yize Zhang \( {}^{1} \) , Zhaoxiang Zhang \( {}^{3} \) , Jiaheng Li \( {}^{1, \dagger  } \)
...
Text-to-Audio-Video (T2AV) generation aims to synthesize temporally coherent video and semantically synchronized audio from natural language, yet its evaluation remains fragmented, often relying on unimodal metrics or narrowly scoped benchmarks that fail to capture cross-modal alignment, instruction following, and perceptual realism under complex prompts. To address this limitation, we present T2AV-Compass \( {}^{ab} \) , a unified benchmark for comprehensive evaluation of T2AV systems, consisting of 500 diverse and complex prompts constructed via a taxonomy-driven pipeline to ensure semantic richness and physical plausibility. Besides, T2AV-Compass introduces a dual-level evaluation framework that integrates objective signal-level metrics for video quality, audio quality, and cross-modal alignment with a subjective MLLM-as-a-Judge protocol for instruction following and realism assessment. Extensive evaluation of 11 representative T2AV systems reveals that even the strongest models fall substantially short of human-level realism and cross-modal consistency, with persistent failures in audio realism, fine-grained synchronization, instruction following, etc. These results indicate significant improvement room for future models and highlight the value of T2AV-Compass as a challenging and diagnostic testbed for advancing text-to-audio-video generation.

---

\( {}^{a} \) https://github.com/NJU-LINK/T2AV-Compass

\( {}^{b} \) https://huggingface.co/datasets/NJU-LINK/T2AV-Compass
...
![019dadf5-c7f6-7111-ba79-240222eb569d_1_199_186_1260_493_0.jpg](images/019dadf5-c7f6-7111-ba79-240222eb569d_1_199_186_1260_493_0.jpg)
...
Benchmarking has evolved from unimodal quality assessment to multimodal, reference-free evaluation of cross-modal consistency (Liu et al., 2025b). Early work on video generation primarily focused on intrinsic visual fidelity and text-video relevance (Yu et al., 2023; Liu et al., 2023; 2024b; He et al., 2024; Tong et al., 2025; Zheng et al., 2025; Duan et al., 2025; Guo et al., 2025). More recent benchmarks extend this paradigm

<table><tr><td>Benchmark</td><td>Task</td><td>Items</td><td>#Metrics</td><td>Avg Tokens./Sub./Events.</td><td>Sound Types</td><td>Eval. Dimensions</td></tr><tr><td>VBench (Yu et al., 2023)</td><td>T2V</td><td>946</td><td>16</td><td>10/1.34/1.06</td><td>-</td><td>VQ</td></tr><tr><td>TTA-Bench (He et al., 2025)</td><td>T2A</td><td>2,999</td><td>10</td><td>20/2.86/1.68</td><td>Sound Music Speech</td><td>AQ</td></tr><tr><td>JavisBench (Liu et al., 2025a)</td><td>T2AV</td><td>10,140</td><td>5</td><td>65/3.68/1.78</td><td>Sound</td><td>VQ AQ CMA</td></tr><tr><td>Verse-Bench (Wang et al., 2025b)</td><td>TI2AV</td><td>600</td><td>4</td><td>68/2.01/1.38</td><td>Sound Speech</td><td>VQ AQ CMA</td></tr><tr><td>Harmony-Bench (Hu et al., 2025)</td><td>TI2AV</td><td>150</td><td>6</td><td>4</td><td>Sound Speech</td><td>VQ AQ CMA</td></tr><tr><td>UniAVGen (Zhang et al., 2025)</td><td>TIA2V</td><td>100</td><td>3</td><td>A</td><td>Speech</td><td>VQ AQ CMA</td></tr><tr><td>VABench (Hua et al., 2025)</td><td>T2AV & I2AV</td><td>778</td><td>15</td><td>50/3.01/2.31</td><td>Sound Music Speech</td><td>VQ AQ CMA</td></tr><tr><td>T2AV-Compass (Ours)</td><td>T2AV</td><td>500</td><td>13</td><td>154/4.03/3.61</td><td>Sound Music Speech</td><td>VQ AQ CMA IF RE</td></tr></table>

Table 1: Comparison of representative generative benchmarks. We provide a detailed comparison of multiple benchmarks in the following dimensions: Avg Tokens/Sub./Events.: Avg Tokens are calculated using the Qwen3 tokenizer(Yang et al., 2025). Sub. refers to the average of distinct themes or subjects addressed in the benchmark dataset. Events indicates the number of events within each subject that are considered for evaluation. Additionally, the table includes a breakdown of sound types in terms of: Sound (general sound), Music (musical content), Speech (speech-related content), where applicable. The evaluation dimensions include: va (Video Quality), AQ (Åudio Quality), CMA (Cross-Modal Alignmen-t/Synchrony), IF (Instruction Following, which includes tasks involving constraints or negations), and Re (Realism Fidelity, which focuses on the perceived accuracy of the generated content beyond general perceptual quality).
...
![019dadf5-c7f6-7111-ba79-240222eb569d_4_185_178_1278_720_0.jpg](images/019dadf5-c7f6-7111-ba79-240222eb569d_4_185_178_1278_720_0.jpg)
...
- Video Aesthetic Score (VA). This metric captures high-level perceptual attributes, including composition, lighting, and color harmony. We utilize the LAION-Aesthetic Predictor V2.5 (Schuh-mann et al., 2022) on extracted keyframes. By averaging these scores, VA serves as a proxy for subjective visual preference and artistic coherence.
...
![019dadf5-c7f6-7111-ba79-240222eb569d_5_189_188_1270_652_0.jpg](images/019dadf5-c7f6-7111-ba79-240222eb569d_5_189_188_1270_652_0.jpg)
...
Instruction Following (IF). This track assesses the model's fidelity to textual prompts. Adopting a decomposition-based strategy, we first derive verifiable QA checklists from each prompt to instantiate abstract instructions into granular, measurable constraints. We employ Gemini-2.5-Pro as the judge to verify the generated video against these checklists. The taxonomy encompasses 7 primary dimensions (including Dynamics, Sound, Cinematography, etc.) decomposed into 17 sub-dimensions:

<table><tr><td rowspan="2">Method</td><td rowspan="2">Open-Source</td><td colspan="2">Video Quality</td><td colspan="2">Audio Quality</td><td colspan="5">Cross-modal Alignment</td></tr><tr><td>VT↑</td><td>VA↑</td><td>PQ↑</td><td>CU↑</td><td>A-V↑</td><td>T-A↑</td><td>T-V↑</td><td>DS↓</td><td>LS↑</td></tr><tr><td colspan="11">- T2AV</td></tr><tr><td>Veo-3.1</td><td>✘</td><td>13.39</td><td>5.425</td><td>7.015</td><td>6.621</td><td>0.2856</td><td>0.2335</td><td>0.2438</td><td>0.6776</td><td>1.509</td></tr><tr><td>Sora-2</td><td>✘</td><td>7.568</td><td>4.112</td><td>5.827</td><td>5.340</td><td>0.2419</td><td>0.2484</td><td>0.2432</td><td>0.8100</td><td>1.331</td></tr><tr><td>Kling-2.6</td><td>✘</td><td>11.41</td><td>5.417</td><td>6.882</td><td>6.449</td><td>0.2495</td><td>0.2495</td><td>0.2449</td><td>0.7852</td><td>1.502</td></tr><tr><td>Wan-2.6</td><td>✘</td><td>11.87</td><td>4.605</td><td>6.658</td><td>6.222</td><td>0.2149</td><td>0.2572</td><td>0.2451</td><td>0.8818</td><td>1.081</td></tr><tr><td>Seedance-1.5</td><td>✘</td><td>12.74</td><td>5.007</td><td>7.555</td><td>7.250</td><td>0.2875</td><td>0.2320</td><td>0.2370</td><td>0.8650</td><td>1.560</td></tr><tr><td>Wan-2.5</td><td>✘</td><td>13.29</td><td>4.642</td><td>6.469</td><td>5.869</td><td>0.2026</td><td>0.2445</td><td>0.2470</td><td>0.8810</td><td>1.065</td></tr><tr><td>Pixverse-V5.5</td><td>✘</td><td>11.54</td><td>4.558</td><td>6.108</td><td>5.855</td><td>0.1816</td><td>0.2305</td><td>0.2431</td><td>0.6627</td><td>1.306</td></tr><tr><td>Ovi-1.1</td><td>✓</td><td>9.336</td><td>4.368</td><td>6.569</td><td>6.492</td><td>0.1620</td><td>0.1756</td><td>0.2391</td><td>0.9624</td><td>1.191</td></tr><tr><td>JavisDiT</td><td>✓</td><td>6.850</td><td>3.575</td><td>4.299</td><td>5.204</td><td>0.1284</td><td>0.1257</td><td>0.2320</td><td>1.322</td><td>-</td></tr><tr><td colspan="11">- \( {T2V} + {TV2A} \)</td></tr><tr><td>Wan-2.2 + Hunyuan-Foley</td><td>✓</td><td>13.43</td><td>5.605</td><td>6.497</td><td>6.208</td><td>0.2575</td><td>0.2076</td><td>0.2455</td><td>0.7935</td><td>-</td></tr><tr><td colspan="11">- T2A + TA2V</td></tr><tr><td>AudioLDM2 + MTV</td><td>✓</td><td>8.066</td><td>3.458</td><td>6.406</td><td>6.100</td><td>0.1639</td><td>0.2698</td><td>0.2394</td><td>1.1592</td><td>0.6835</td></tr></table>
...
- The Gap Between Open and Closed-Source. Closed-source models show superior performance over open-source ones in both objective metrics and semantic evaluations. (Table 2, 3).

<table><tr><td>Method</td><td>Open-Source</td><td>IF Video↑</td><td>IF Audio↑</td><td>Video Realism↑</td><td>Audio Realism↑</td><td>Average↑</td></tr><tr><td colspan="7">- T2AV</td></tr><tr><td>Veo-3.1</td><td>✘</td><td>76.15</td><td>67.90</td><td>87.14</td><td>49.95</td><td>70.29</td></tr><tr><td>Sora-2</td><td>✘</td><td>74.93</td><td>72.86</td><td>85.53</td><td>46.01</td><td>69.83</td></tr><tr><td>Kling-2.6</td><td>✘</td><td>73.72</td><td>63.89</td><td>87.98</td><td>47.03</td><td>68.16</td></tr><tr><td>Wan-2.6</td><td>✘</td><td>78.52</td><td>74.95</td><td>82.05</td><td>35.18</td><td>67.68</td></tr><tr><td>Seedance-1.5</td><td>✘</td><td>60.96</td><td>61.22</td><td>88.94</td><td>53.84</td><td>66.24</td></tr><tr><td>Wan-2.5</td><td>✘</td><td>76.56</td><td>57.95</td><td>76.00</td><td>35.06</td><td>61.39</td></tr><tr><td>Pixverse-V5.5</td><td>✘</td><td>65.13</td><td>53.31</td><td>69.37</td><td>33.58</td><td>55.35</td></tr><tr><td>Ovi-1.1</td><td>✓</td><td>55.05</td><td>52.83</td><td>65.93</td><td>30.75</td><td>51.14</td></tr><tr><td>JavisDiT</td><td>✓</td><td>32.56</td><td>15.26</td><td>34.97</td><td>14.85</td><td>24.41</td></tr><tr><td colspan="7">- \( {T2V} + {TV2A} \)</td></tr><tr><td>Wan-2.2 + Hunyuan-Foley</td><td>✓</td><td>64.54</td><td>38.19</td><td>89.63</td><td>41.25</td><td>58.40</td></tr><tr><td colspan="7">- T2A + TA2V</td></tr><tr><td>AudioLDM2 + MTV</td><td>✓</td><td>47.13</td><td>54.39</td><td>56.73</td><td>31.90</td><td>47.54</td></tr></table>
...
![019dadf5-c7f6-7111-ba79-240222eb569d_7_191_1220_1265_343_0.jpg](images/019dadf5-c7f6-7111-ba79-240222eb569d_7_191_1220_1265_343_0.jpg)
...
Xiaoxuan Tang, Xinping Lei, Chaoran Zhu, Shiyun Chen, Ruibin Yuan, Yizhi Li, Changjae Oh, Ge Zhang, Wenhao Huang, Emmanouil Benetos, Yang Liu, Jiaheng Liu, and Yinghao Ma. Automox: An automatic multi-agent system for music video generation, 2025. URL https://arxiv.org/abs/2512.12196.
...
The observed "Audio Realism Bottleneck" suggests that future research should prioritize native audiovisual joint-diffusion architectures over traditional composed models to better capture cross-modal physical correlations. We plan to extend our benchmark to support long-duration video evaluation (> 10 seconds) and develop distilled, lightweight evaluators to reduce costs. Furthermore, incorporating human-in-the-loop feedback will be essential to further align our automated diagnostic protocols with nuanced human perception, fostering more physically grounded T2AV generation.

## C More Detailed Results

## D MLLM Evaluation Framework

The following table provides a detailed breakdown of these dimensions and sub-dimensions, offering a comprehensive approach to realism evaluation.

<table><tr><td rowspan="2">Method</td><td rowspan="2">Open</td><td colspan="2">Attribute</td><td colspan="3">Cinematography</td><td colspan="4">Sound</td></tr><tr><td>Look↑</td><td>Quantity↑</td><td>Light↑</td><td>Frame↑</td><td>ColorGrading↑</td><td>SFX↑</td><td>Speech↑</td><td>Music↑</td><td>NonSpeech↑</td></tr><tr><td colspan="11">- T2AV (end-to-end)</td></tr><tr><td>Veo-3.1</td><td>-</td><td>0.8975</td><td>0.7500</td><td>0.7870</td><td>0.7282</td><td>0.8500</td><td>0.5798</td><td>0.7939</td><td>0.7096</td><td>0.9537</td></tr><tr><td>Sora-2</td><td>-</td><td>0.8597</td><td>0.7990</td><td>0.8591</td><td>0.7369</td><td>0.7704</td><td>0.6373</td><td>0.8802</td><td>0.8135</td><td>0.8548</td></tr><tr><td>Kling-2.6</td><td>-</td><td>0.8268</td><td>0.7710</td><td>0.8983</td><td>0.7673</td><td>0.8864</td><td>0.5902</td><td>0.8073</td><td>0.5466</td><td>0.9727</td></tr><tr><td>Wan-2.6</td><td>-</td><td>0.9317</td><td>0.7786</td><td>0.8333</td><td>0.7863</td><td>0.9034</td><td>0.6801</td><td>0.8957</td><td>0.7231</td><td>0.9496</td></tr><tr><td>SeeDance-1.5</td><td>-</td><td>0.6143</td><td>0.5967</td><td>0.5647</td><td>0.6187</td><td>0.6898</td><td>0.6480</td><td>0.6020</td><td>0.6976</td><td>0.8889</td></tr><tr><td>Wan-2.5</td><td>-</td><td>0.8924</td><td>0.7698</td><td>0.7977</td><td>0.7302</td><td>0.8316</td><td>0.6629</td><td>0.7162</td><td>0.6204</td><td>0.7391</td></tr><tr><td>PixVerse-V5.5</td><td>-</td><td>0.7375</td><td>0.5434</td><td>0.7449</td><td>0.6831</td><td>0.6389</td><td>0.4816</td><td>0.8050</td><td>0.5407</td><td>0.7719</td></tr><tr><td>Ovi-1.1</td><td>✓</td><td>0.6265</td><td>0.5164</td><td>0.7203</td><td>0.5681</td><td>0.7864</td><td>0.2934</td><td>0.6299</td><td>0.6655</td><td>0.9961</td></tr><tr><td>JavisDiT</td><td>✓</td><td>0.3636</td><td>0.3962</td><td>0.4295</td><td>0.4613</td><td>0.4907</td><td>0.2987</td><td>0.0267</td><td>0.1591</td><td>0.9653</td></tr><tr><td colspan="11">- \( {T2V} + {TV2A} \)</td></tr><tr><td>Wan-2.2 + Hunyuan-Foley</td><td>✓</td><td>0.7636</td><td>0.6565</td><td>0.8030</td><td>0.7141</td><td>0.7182</td><td>0.4835</td><td>0.1089</td><td>0.6621</td><td>0.9248</td></tr><tr><td colspan="11">- \( {T2}A + {TA}{2V} \)</td></tr><tr><td>AudioLDM2 + MTV</td><td>✓</td><td>0.6581</td><td>0.5537</td><td>0.5826</td><td>0.6101</td><td>0.5727</td><td>0.3988</td><td>0.4818</td><td>0.7707</td><td>0.9805</td></tr></table>

<table><tr><td rowspan="2">Method</td><td rowspan="2">Open</td><td colspan="4">Dynamics</td><td colspan="2">Relations</td><td colspan="2">Aesthetics</td><td>World Knowledge</td></tr><tr><td>Camera Motion↑</td><td>Interaction↑</td><td>Motion↑</td><td>Transform↑</td><td>Spatial↑</td><td>Logical↑</td><td>Style↑</td><td>Mood↑</td><td>Factual↑</td></tr><tr><td colspan="11">- T2AV (end-to-end)</td></tr><tr><td>Veo-3.1</td><td>-</td><td>0.5250</td><td>0.6872</td><td>0.6748</td><td>0.5420</td><td>0.7215</td><td>0.7852</td><td>0.8134</td><td>0.8750</td><td>0.7518</td></tr><tr><td>Sora-2</td><td>-</td><td>0.4365</td><td>0.6562</td><td>0.6080</td><td>0.5469</td><td>0.7647</td><td>0.6741</td><td>0.7409</td><td>0.8459</td><td>0.8029</td></tr><tr><td>Kling-2.6</td><td>-</td><td>0.6396</td><td>0.6721</td><td>0.6176</td><td>0.5765</td><td>0.7363</td><td>0.7129</td><td>0.7783</td><td>0.8567</td><td>0.6052</td></tr><tr><td>Wan-2.6</td><td>-</td><td>0.7562</td><td>0.6550</td><td>0.6179</td><td>0.5766</td><td>0.7671</td><td>0.7518</td><td>0.7424</td><td>0.8713</td><td>0.7972</td></tr><tr><td>SeeDance-1.5</td><td>-</td><td>0.6762</td><td>0.6608</td><td>0.5912</td><td>0.5564</td><td>0.5312</td><td>0.5308</td><td>0.7466</td><td>0.8188</td><td>0.4930</td></tr><tr><td>Wan-2.5</td><td>-</td><td>0.7432</td><td>0.6769</td><td>0.6002</td><td>0.5712</td><td>0.7785</td><td>0.7776</td><td>0.7255</td><td>0.8600</td><td>0.7574</td></tr><tr><td>PixVerse-V5.5</td><td>-</td><td>0.5916</td><td>0.7118</td><td>0.6452</td><td>0.4688</td><td>0.6974</td><td>0.6489</td><td>0.6552</td><td>0.8152</td><td>0.5654</td></tr><tr><td>Ovi-1.1</td><td>✓</td><td>0.5311</td><td>0.4068</td><td>0.3944</td><td>0.3582</td><td>0.6055</td><td>0.5081</td><td>0.5417</td><td>0.7378</td><td>0.4207</td></tr><tr><td>JavisDiT</td><td>✓</td><td>0.1245</td><td>0.1860</td><td>0.2034</td><td>0.1504</td><td>0.2500</td><td>0.2697</td><td>0.2735</td><td>0.4219</td><td>0.3392</td></tr><tr><td colspan="11">- \( {T2V} + {TV2A} \)</td></tr><tr><td>Wan-2.2 + Hunyuan-Foley</td><td>✓</td><td>0.5679</td><td>0.5010</td><td>0.4648</td><td>0.3899</td><td>0.7285</td><td>0.6145</td><td>0.6717</td><td>0.8201</td><td>0.5190</td></tr><tr><td colspan="11">- T2A + TA2V)</td></tr><tr><td>AudioLDM2 + MTV</td><td>✓</td><td>0.2792</td><td>0.2941</td><td>0.3259</td><td>0.2985</td><td>0.4766</td><td>0.4774</td><td>0.4033</td><td>0.5518</td><td>0.3793</td></tr></table>

Table 5: Video Instruction Following

<table><tr><td>Dimension</td><td>Sub-dimension</td><td>Definition</td></tr><tr><td>World Knowledge</td><td>Factual Knowledge</td><td>Accurate depiction of inherent characteristics of specific entities, landmarks, and historical/cultural symbols</td></tr><tr><td rowspan="2">Attribute</td><td>Look</td><td>Visual appearance including color, shape, size, material, expression, and physical state</td></tr><tr><td>Quantity</td><td>Statistical count of specific objects in the frame</td></tr><tr><td rowspan="3">Cinematography</td><td>Light</td><td>Light sources, lighting types (backlighting, volumetric light), and light/shadow texture</td></tr><tr><td>Frame</td><td>Shot size, lens settings, shooting angle, and framing methods</td></tr><tr><td>Color Grading</td><td>Color tendency, saturation, and contrast style</td></tr><tr><td rowspan="4">Dynamics</td><td>Motion</td><td>Specific behaviors, speed, and trajectory characteristics of objects</td></tr><tr><td>Interaction</td><td>Contact or reactions between multiple entities</td></tr><tr><td>Transformation</td><td>Changes in essential attributes or state evolution over time</td></tr><tr><td>Camera Motion</td><td>Movement methods of the camera (dolly, pan, track, etc.)</td></tr><tr><td rowspan="2">Relations</td><td>Spatial</td><td>Physical positional relationships in 2D plane and 3D depth</td></tr><tr><td>Logical</td><td>Abstract semantic connections including composition, comparison, and inclusion</td></tr><tr><td rowspan="2">Aesthetics</td><td>Style</td><td>Visual expression form, artistic genre, or medium texture</td></tr><tr><td>Mood</td><td>Overall emotional tone or environmental atmosphere</td></tr></table>

Table 6: Audio Instruction Following

<table><tr><td>Dimension</td><td>Sub-dimension</td><td>Definition</td></tr><tr><td rowspan="3">Sound</td><td>Sound Effects</td><td>Ambient atmosphere sounds and specific physical sounds triggered by actions</td></tr><tr><td>Speech</td><td>Human oral expression including dialogue, monologue, and voiceover</td></tr><tr><td>Music</td><td>Music-related elements including instruments, genres, rhythm, and emotion</td></tr></table>
...
Table 7: Technical Quality Evaluation Framework: Five Scoring Dimensions and Sub-dimensions

<table><tr><td>Dimension</td><td>Sub-dimension</td><td>Definition</td></tr><tr><td rowspan="3">TCS</td><td>Existence Continuity</td><td>Detecting abnormal disappearance, appearance, and flickering of objects</td></tr><tr><td>Identity Stability</td><td>Examining sudden changes in object category or appearance features</td></tr><tr><td>Occlusion & Boundary Logic</td><td>Verifying logical consistency of object occlusion and frame entry/exit</td></tr><tr><td rowspan="3">OIS</td><td>Biological Anatomical Constraints</td><td>Consistency of limb length, joint angles, and facial features</td></tr><tr><td>Rigid Body Rigidity</td><td>Geometric shape preservation and contour stability of rigid objects</td></tr><tr><td>Texture & Semantic Consistency</td><td>Frame-to-frame consistency of texture details and surface patterns</td></tr><tr><td rowspan="3">MTC</td><td>Material-Timbre Matching</td><td>Accuracy of sound timbre in representing material properties</td></tr><tr><td>Interaction Dynamics</td><td>Correspondence between sound envelope and action intensity</td></tr><tr><td>Environmental Acoustics</td><td>Matching of reverb and echo with visual spatial characteristics</td></tr><tr><td rowspan="3">MSS</td><td>Artifacts & Degradation</td><td>Detection of unnatural blur, pixel blocks, and flickering</td></tr><tr><td>Fluidity of Motion</td><td>Smoothness of frame transitions and optical flow consistency</td></tr><tr><td>Scene-Aware Analysis</td><td>Context-appropriate evaluation of motion blur based on scene dynamics</td></tr><tr><td rowspan="3">AAS</td><td>Generative Artifacts</td><td>Detection of metallic sound, smearing, and frequency truncation</td></tr><tr><td>Temporal Stability</td><td>Detection of pops, clicks, dropouts, and noise floor pumping</td></tr><tr><td>Signal Integrity</td><td>Detection of clipping distortion and hallucinated noises</td></tr></table>
...
You are an expert in Text-to-Audio/Video model prompt optimization. Your task is to receive the

\( \rightarrow \) original Text-to-Audio/Video prompt provided by the user, and strictly follow the detailed

\( \leftrightarrow \) rewriting guidelines below to rewrite it into a structured, visually strong, and naturally

\( \hookrightarrow \) fluent professional-level prompt. The rewritten prompt should be output directly without any

	\( \hookrightarrow \) pre or post explanatory text.

**Core Rewriting Principles:**

	1. **Absolute Fidelity to Original and Adaptive Enhancement**: Your primary principle is to

							\( \hookrightarrow \) maximize the retention of the original prompt's core creativity, theme, and emotion. Your work

				\( \hookrightarrow \) is to "enhance" rather than "replace". Please dynamically decide which core description

\( \hookrightarrow \) modules need to be selected based on the core concept of the original Prompt, and make

\( \hookrightarrow \) reasonable and creative expansions.

		2. **Distinguish Prompt from API Parameters**: You must understand that video **resolution

			\( \leftrightarrow \) (size)**, **aspect ratio**, and **duration (seconds)** are directly controlled by API call

	\( \hookrightarrow \) parameters. Therefore, in the rewritten prompt, **strictly prohibit** any descriptions that

	\( \leftrightarrow \) specify or imply these parameters. Your prompt must focus on all other factors: **subject,

- scene, dynamics, cinematography, style, and sound**.
...
\( \hookrightarrow \) modules based on specific situations to construct a natural, fluent, and logically coherent
...
1. **Subject (Subject Description) [Core Element]:**
	...
	* **Method:** Use descriptive phrases, not just nouns, to define its **appearance, clothing,

		\( \hookrightarrow \) features, and essence**. For example, refine "a girl" to "**A black-haired Miao girl

		\( \hookrightarrow \) wearing intricate ethnic minority clothing**"; refine "a monster" to "**A flying fairy

		\( \leftrightarrow \) from another world, dressed in tattered yet elegant attire, with a pair of strange wings

		\( \hookrightarrow \) made of rubble fragments**".

2. **Scene (Scene Description) [Core Element]:**

		* **Goal:** Build an environment with depth and atmosphere.

	* **Method:** Specifically describe **environment, background, foreground, and light

	\( \hookrightarrow \) atmosphere**. For example, "**A sunlit Roman square with actors seated around a marble

		\( \hookrightarrow \) table, a horse-drawn carriage passing on the cobblestone street in the background**".
	...
	* **Method:** Comprehensively use vivid verbs and professional camera movement terminology to
		...
		- precisely describe **subject/object actions** and **camera movement**.
			...
			* **Movement:** Refine specific dynamic behaviors of subjects or objects. For example,
			...
			\( \hookrightarrow \) "**The player holds the ball with both hands and executes an explosive two-handed dunk

			\( \hookrightarrow \) with tremendous force**".

			* **Camera Motion:** Define camera perspective changes and movement trajectories. For

			- example: "**Pan left/right**", "**Tilt up/down**", "**Zoom in/out**", "**Tracking
...
4. **Cinematography (Cinematographic Control) [Optional Enhancement]:**
	...
	* **Method:** This is a technical module that can **selectively** describe from one or more
		...
		\( \hookrightarrow \) of the following categories:
			...
			* **Color:** Specify color tone and saturation (e.g., `warm colors`, `cool colors`,
			...
			\( \hookrightarrow \) `saturated colors`, `desaturated colors`).
...
5. **Stylization (Stylization) [Optional Enhancement]:**
		...
		* **Method:** **Selectively** use one or a set of clear style keywords. For example:
		...
		\( \hookrightarrow \) "**Cyberpunk**", "**Watercolor painting style**", "**3D cartoon style**", "**Claymation
...
6. **Sound (Sound Description) [Core Element]:**
	...
	* **Method:** **Selectively** describe from one or more of the following aspects, which must
		...
		\( \hookrightarrow \) highly match the visual atmosphere of the scene:

			* **Voice:** Describe dialogue content, tone emotion, and speech rate. For example: "**A

			\( \hookrightarrow \) man is talking about his insomnia. He says,'love is not getting but giving.' The tone

			\( \hookrightarrow \) is relaxed, the pace is moderate, the voice is bright and clear, in American

			\( \hookrightarrow \) English.**"

			* **Sound Effects:** Describe sound source actions and sound effect content. For example:

			- "**A piece of glass falls from the table onto a wooden floor, making a 'shatter' sound,

			\( \hookrightarrow \) in a quiet indoor environment.**"

			* **Background Music (BGM):** Describe music content and its style and emotion. For

			\( \hookrightarrow \) example: "**On a rainy night, in a gloomy, narrow corridor, suspense-style background

			\( \hookrightarrow \) music plays.**"
...
\( \hookrightarrow \) text paragraph** that seamlessly integrates all selected module content.

**Rewriting Examples:**

**Example 1:**

In a medium shot, historical adventure setting, warm lamplight illuminates a cartographer in a

\( \rightarrow \) cluttered study. He is deeply engrossed in a sprawling ancient map spread across a large table.

	\( \leftrightarrow \) Breaking the silence, he exclaims,"According to this old sea chart, the lost island isn't

- myth! We must prepare an expedition immediately!"

**Example 2:**

A seasoned, grey-bearded man in sunglasses and a paisley shirt, his gaze fixed off-camera with a

		\( \hookrightarrow \) contemplative expression. His gold chain glints subtly. The camera slowly pushes in, subtly

\( \hookrightarrow \) emphasizing his quiet focus. In the background, a vibrant mural splashes across a wall,

	\( \rightarrow \) hinting at an urban setting. Faint city murmurs and distant chatter drift in, accompanied by a

		\( \leftrightarrow \) mellow, soulful hip-hop beat that adds a contemplative yet grounded atmosphere. "The city

		\( \leftrightarrow \) always got a story," the older man murmurs, a slight nod of his head. "Just gotta listen."

**Example 3:**

	In a brightly sunlit bedroom, a joyful 5-year-old girl with curly blonde pigtails and a

			\( \rightarrow \) paint-smudged pink dress enthusiastically turns a large white wall into her canvas. The

		\( \hookrightarrow \) surface is vibrantly covered with whimsical, childlike scribbles as she drags a red crayon

	\( \leftrightarrow \) across it, leaving a thick, waxy trail. She giggles softly with pure delight, admiring her

\( \hookrightarrow \) creation. The scene is captured with cinematic realism and a heartwarming style, featuring

							\( \hookrightarrow \) highly saturated colors, soft warm natural lighting, and a shallow depth of field. The camera

		\( \hookrightarrow \) begins with an eye-level medium shot and performs a slow dolly-in, transitioning to a close-up

	\( \hookrightarrow \) of her beaming face. The sound design blends the innocent giggles of the girl with the gentle,

	\( \hookrightarrow \) scratchy sound of the crayon on the wall.

**Execution Instructions:**

Now, please receive the original prompt provided by the user below, and strictly follow the core

\( \hookrightarrow \) principles and detailed guidelines above, directly output the rewritten **fully English**

	\( \hookrightarrow \) prompt.
...
You are a prompt-writing expert specializing in text-to-audio-video generation models. Your task

\( \hookrightarrow \) is to take the user's provided raw audio-video and, by strictly following the detailed

\( \hookrightarrow \) guidelines below, rewrite it into a structured, vivid, and natural professional-grade prompt.

\( \hookrightarrow \) Output the rewritten prompt directly, with no preface or postscript explanations.

**Core principles:**

1. **Faithfulness to the original with adaptive detailing:** Your top priority is to describe the

\( \hookrightarrow \) core content, theme, and emotion of the original audio-video as accurately as possible. Your

\( \hookrightarrow \) job is to *describe*, not to *invent*. Based on the video's core content, dynamically decide

\( \hookrightarrow \) which description modules are necessary and describe them in a reasonable and accurate manner.

2. **Separate prompt text from API parameters:** Note that video **resolution (size)**, **aspect

\( \leftrightarrow \) ratio**, and **duration (seconds)** are controlled by the API call parameters. Therefore, the

\( \hookrightarrow \) prompt must **never** specify or imply any of these parameters. The prompt should focus on all

</softer factors: **subject, scene, motion, cinematography, style, and sound**.

**Detailed guidelines and methodology:**

You will analyze the provided audio-video and selectively integrate the following six core modules

\( \hookrightarrow \) depending on the scenario, forming a single coherent paragraph.
...
1. **Subject (core):**
	...
	- **Goal:** Clearly describe the focal subject.
		...
		\( \hookrightarrow \) distinctive traits, and essence**. For example, refine "a girl" into "**A black-haired Miao

		\( \hookrightarrow \) girl wearing intricate ethnic minority clothing**"; refine "a monster" into "**A flying

		\( \hookrightarrow \) fairy from another world, dressed in tattered yet elegant attire, with a pair of strange

		\( \hookrightarrow \) wings made of rubble fragments**".

2. **Scene (core):**

		- **Goal:** Build an environment with depth and atmosphere.

		- **Method:** Describe the **environment, background, foreground, and lighting ambience** with

		\( \leftrightarrow \) concrete details. For example, ```**A sunlit Roman square with actors seated around a marble
...
3. **Motion (core):**
		...
		\( \hookrightarrow \) **actions of the subject/objects** and **camera movement**.
...
6. **Sound (core):**
		...
		\( \hookrightarrow \) atmosphere:
			...
			- **Voice:** Describe dialogue content, emotion, and speaking pace. Example: ```**A man is

			\( \hookrightarrow \) talking about his insomnia. He says, 'love is not getting but giving.' The tone is

			\( \hookrightarrow \) relaxed, the pace is moderate, the voice is bright and clear, in American English.**"

			- **Sound Effects:** Describe the sound source and effect. Example: ```**A piece of glass falls

			\( \hookrightarrow \) from the table onto a wooden floor, making a ’shatter’ sound, in a quiet indoor
...
Strictly follow an integrated format. The final output must be a **single, fluent, natural

\( \hookrightarrow \) paragraph** that seamlessly integrates all selected modules.
...
\( \hookrightarrow \) cluttered study. He is deeply engrossed in a sprawling ancient map spread across a large table.

\( \hookrightarrow \) Breaking the silence, he exclaims,"According to this old sea chart, the lost island isn't

\( \hookrightarrow \) myth! We must prepare an expedition immediately!"

**Example 2:**

A seasoned, grey-bearded man in sunglasses and a paisley shirt, his gaze fixed off-camera with a

\( \rightarrow \) contemplative expression. His gold chain glints subtly. The camera slowly pushes in, subtly

\( \hookrightarrow \) emphasizing his quiet focus. In the background, a vibrant mural splashes across a wall,

		\( \hookrightarrow \) hinting at an urban setting. Faint city murmurs and distant chatter drift in, accompanied by a

\( \hookrightarrow \) mellow, soulful hip-hop beat that adds a contemplative yet grounded atmosphere. "The city
...
\( \hookrightarrow \) paint-smudged pink dress enthusiastically turns a large white wall into her canvas. The

\( \hookrightarrow \) surface is vibrantly covered with whimsical, childlike scribbles as she drags a red crayon

\( \leftrightarrow \) across it, leaving a thick, waxy trail. She giggles softly with pure delight, admiring her

\( \leftrightarrow \) creation. The scene is captured with cinematic realism and a heartwarming style, featuring
...
\( \hookrightarrow \) guidelines above. Output the rewritten **fully English** prompt directly.
...
\( \hookrightarrow \) binary question-answer pairs (Binary QA) for automated evaluation based on the user's input

\( \hookrightarrow \) Prompt, focusing on the **Aesthetics** dimension.
...
\( \hookrightarrow \) specific emotional atmospheres. Please analyze based on the following sub-dimension

\( \hookrightarrow \) definitions:

1. **Style (Style)**

		- **Definition**: Describes the visual expression form, artistic genre, or medium texture of

		\( \hookrightarrow \) the frame.
...
2. **Mood (Atmosphere/Emotion)**
	...
	- **Note**: *Do not test emotions and atmospheres related to sound here, as those belong to the
		...
		\( \hookrightarrow \) Sound dimension. This section only focuses on the atmosphere and emotions of visual

		\( \hookrightarrow \) content.*

		- **Generation Strategy**: Extract adjectives describing emotions or atmosphere from the Prompt,

		\( \hookrightarrow \) and generate **1 core question**.

#Task Instructions

1. **Analyze Prompt**: Carefully read the Text-to-Audio/Video Prompt and identify keywords

\( \hookrightarrow \) defining visual style (Style) and emotional tone (Mood).

2. **Generate QA**:

---

- For the two sub-dimensions **Style** and **Mood**, generate **only 1 binary question** \( \hookrightarrow \) (Yes/No Question) **per sub-dimension**.

- Questions should be as objective as possible, directly asking whether the style features or \( \hookrightarrow \) atmosphere are present.

- The expected answer for questions must be **"Yes"** (i.e., assuming the video perfectly \( \hookrightarrow \) presents the aesthetic requirements).
...
4. **Output Format**: Return a valid JSON object directly, strictly prohibit including Markdown \( \hookrightarrow \) markers or explanatory text.
...
\( \hookrightarrow \) design binary question-answer pairs (Binary QA) for automated evaluation based on the user's

\( \hookrightarrow \) input Prompt, focusing on the **Attribute** specific dimension.
...
\( \hookrightarrow \) **visual entities** in videos. Please analyze based on the following sub-dimension definitions:
...
1. **Look (Appearance)**
	...
	\( \leftrightarrow \) Prompt (such as "red","huge","round","wooden","crying","broken"), and integrate them
...
2. **Generate QA**:
	...
	\( \hookrightarrow \) (Yes/No Question) **per sub-dimension**.
	...
	\( \hookrightarrow \) matches the Prompt description).

3. **Default Handling**: If the Prompt does not explicitly mention features for a certain

\( \hookrightarrow \) sub-dimension (e.g., only says "a cat" without appearance, or no specific quantity mentioned),

\( \rightarrow \) then **do not generate** a question for that sub-dimension, and set the corresponding value in

\( \hookrightarrow \) JSON to `null`.

4. **Output Format**: Return a valid JSON object directly, strictly prohibit including Markdown

\( \hookrightarrow \) markers or any explanatory text.
...
\( \hookrightarrow \) Prompt, focusing on the **Cinematography** dimension.
...
\( \hookrightarrow \) or "cinematographer". Please analyze based on the following sub-dimension definitions:

1. **Light (Lighting)**

	- **Definition**: Involves light sources (natural light, artificial light), lighting types

	\( \hookrightarrow \) (backlighting, side lighting, volumetric light/Tyndall effect), and light/shadow texture

	\( \hookrightarrow \) (soft light, hard light).

	- **Generation Strategy**: Extract descriptions about light environment or lighting methods

	\( \hookrightarrow \) from the Prompt, and generate **1 core question**.

2. **Frame (Framing)**

	- **Definition**: Involves shot size (close-up, medium shot, wide shot), lens settings (wide

	\( \hookrightarrow \) angle, telephoto, depth of field/bokeh), shooting angle (overhead, low angle, eye level),
	...
	\( \hookrightarrow \) the Dynamics dimension. This section only focuses on lens settings and static angles.*

	- **Generation Strategy**: Generate **1 core question** regarding shot size, optical

	\( \hookrightarrow \) characteristics of the lens, shooting angle, or framing layout.

3. **ColorGrading (Color Grading)**

	- **Definition**: Involves color tendency (cool/warm tones, black and white, vintage tones),

	\( \hookrightarrow \) saturation, and contrast style (high contrast, low contrast, film noir style).

	- **Generation Strategy**: Generate **1 core question** regarding the overall color tone or

	\( \hookrightarrow \) color style of the frame.

#Task Instructions

1. **Analyze Prompt**: Carefully read the Text-to-Audio/Video Prompt and identify descriptive

\( \hookrightarrow \) words belonging to cinematographic language.

2. **Generate QA**:

	- For the three sub-dimensions **Light**, **Frame**, and **ColorGrading**, generate **only 1

	\( \hookrightarrow \) binary question** (Yes/No Question) **per sub-dimension**.

	- The expected answer for questions must be **"Yes"** (i.e., assuming the video perfectly

	\( \hookrightarrow \) presents the cinematographic requirements in the Prompt).

3. **Default Handling**: If the Prompt does not mention features for a certain sub-dimension (e.g.,

\( \leftrightarrow \) only describes action without specifying light or shot size), then **do not generate** a

\( \leftrightarrow \) question for that sub-dimension, and set the corresponding value in JSON to `null`.

4. **Output Format**: Return a valid JSON object directly, strictly prohibit including Markdown

\( \hookrightarrow \) markers or explanatory text.
...
\( \hookrightarrow \) Prompt, focusing on the **Dynamics** core dimension.
...
\( \hookrightarrow \) analyze based on the following sub-dimension definitions:

1. **Motion (Motion)**

	- **Definition**: Describes **specific behaviors** performed by objects, as well as **physical

	\( \hookrightarrow \) properties** or **trajectory characteristics** when moving in space.

	- **Focus Points**: Verbs (running, waving, dancing), speed (fast, slow motion), motion

	\( \hookrightarrow \) trajectory (straight sprint, spiral ascent).

	- **Generation Strategy**: Generate **1 core question** regarding the object's core behavior,

	\( \hookrightarrow \) speed, or trajectory characteristics.
...
2. **Interaction (Interaction)**
	...
	\( \hookrightarrow \) multiple subjects.
...
3. **Transformation (Transformation)**
	...
	\( \hookrightarrow \) changes occurring over time.
...
4. **CameraMotion (Camera Motion)**
	...
	\( \hookrightarrow \) camera movement.*
...
\( \hookrightarrow \) involving temporal changes and motion.
...
2. **Generate QA**:
	...
	\( \hookrightarrow \) sub-dimension**.

	- Questions must target dynamic processes (i.e., those occurring during video playback), not

	\( \hookrightarrow \) just static frames.

	- The expected answer for questions must be **"Yes"** (i.e., assuming the video perfectly

	\( \hookrightarrow \) presents the dynamic requirements).

3. **Default Handling**: If the Prompt does not mention dynamics for a certain sub-dimension (e.g.,

\( \leftrightarrow \) only motion without camera motion description), then **do not generate** a question for that

\( \hookrightarrow \) sub-dimension, and set the corresponding value in JSON to `null`.
...
\( \hookrightarrow \) Prompt, focusing on the **Relations** dimension.
...
\( \hookrightarrow \) elements. Please analyze based on the following sub-dimension definitions:
...
1. **Spatial (Spatial Relations)**
	...
	\( \hookrightarrow \) layouts in the Prompt, and generate **1 core question**.
...
2. **Logical (Logical Relations)**
	...
	- **Definition**: Describes abstract semantic or structural connections between objects.
		...
		\( \hookrightarrow \) house with a red roof"). *Note: Focus on the correctness of component attribution.*
		...
		\( \hookrightarrow \) cage","the moon reflected in water").

	- **Generation Strategy**: Generate **1 core question** regarding ownership, comparison, or

	\( \hookrightarrow \) inclusion relationships between objects.

#Task Instructions

1. **Analyze Prompt**: Carefully read the Text-to-Audio/Video Prompt and identify statements

\( \hookrightarrow \) describing positional layouts or logical connections between objects.
...
2. **Generate QA**:
	...
	\( \hookrightarrow \) presents the relationship description).

3. **Default Handling**: If the Prompt does not mention a certain type of relationship (e.g., only

\( \rightarrow \) describes a single object with no background position or compositional details), then **do not

\( \hookrightarrow \) generate** a question for that sub-dimension, and set it to `null` in JSON.
...
\( \hookrightarrow \) Prompt, focusing on the **Sound** dimension.

#Evaluation Dimension: Sound

This dimension aims to test the model's ability to generate specific auditory elements. Please

\( \hookrightarrow \) analyze based on the following sub-dimension definitions:

1. **SoundEffects (Sound Effects)**

	- **Definition**: Covers ambient atmosphere sounds (such as wind, rain, urban noise) and

	\( \hookrightarrow \) specific physical sounds triggered by actions (such as footsteps, engine roar, object

	\( \hookrightarrow \) collisions, animal calls).

	- **Generation Strategy**: Extract specific sounds or ambient sound effects described in the

	\( \hookrightarrow \) Prompt, and generate **1 core question**.

2. **Speech (Speech)**

	- **Definition**: Involves human oral expression, including dialogue, monologue, and voiceover.

	\( \hookrightarrow \) Focus on content (specific lines), language, accent, or speaker's voice characteristics.

	- **Generation Strategy**: Generate **1 core question** regarding speech content, language, or

	\( \hookrightarrow \) speaking style.

3. **Music (Music)**

	- **Definition**: Covers all music-related elements, including background music (BGM),

	\( \hookrightarrow \) instrumental performance, and vocal singing with melody and lyrics. Focus on instrument

	\( \hookrightarrow \) types (piano, guitar), music genres (rock, jazz, classical), rhythm (fast/slow), emotion

	\( \hookrightarrow \) (sad, exciting), and specific singing behavior or lyric content.

	- **Generation Strategy**: Generate **1 core question** regarding music style, instruments,

	\( \hookrightarrow \) emotion, or singing content.

#Task Instructions

1. **Analyze Prompt**: Carefully read the Text-to-Audio/Video Prompt and identify explicitly

\( \hookrightarrow \) specified auditory elements.
...
2. **Generate QA**:
	...
	\( \hookrightarrow \) video perfectly matches the Prompt description).

3. **Default Handling**: If the Prompt does not mention sound for a certain sub-dimension (e.g.,

\( \hookrightarrow \) only describes music but no speech), then **do not generate** a question for that
...
\( \leftrightarrow \) binary question-answer pairs (Binary QA) for automated evaluation based on the user's input

\( \hookrightarrow \) Prompt, focusing on the **World Knowledge** dimension.

#Evaluation Dimension: World Knowledge

This dimension aims to test the model's understanding and ability to accurately represent

\( \hookrightarrow \) objective facts and common knowledge about the real world. Please analyze based on the

- following sub-dimension definitions:
...
1. **FactualKnowledge (Factual Knowledge)**
	...
	\( \hookrightarrow \) of specific entities, landmarks, historical or cultural symbols in the described world.

	- **Core Logic**: Focus on features that entities "naturally possess". For example: If the

	\( \hookrightarrow \) Prompt mentions "panda", even without specifying color, the question should be "Is the

	\( \hookrightarrow \) panda black and white?"; If the Prompt mentions "Eiffel Tower", the question should involve

	\( \hookrightarrow \) its unique tower structure.

	- **Generation Strategy**: Identify proper nouns in the Prompt, and based on recognized factual

	\( \hookrightarrow \) knowledge, generate **1 question** to verify whether the inherent characteristics of that

	\( \hookrightarrow \) entity are accurately depicted.

#Task Instructions

1. **Analyze Prompt**: Carefully read the Text-to-Audio/Video Prompt and identify scenarios

\( \hookrightarrow \) involving world knowledge.
...
2. **Generate QA**:
	...
	- Questions must be **objectively verifiable**. For example, do not ask "Does the video depict

	\( \hookrightarrow \) the Atlantic Ocean?" because scenes of the Atlantic Ocean are difficult to verify

	\( \hookrightarrow \) objectively.

	- The expected answer for questions must be **"Yes"** (i.e., assuming the video perfectly

	\( \hookrightarrow \) satisfies world facts).

3. **Default Handling**: If the Prompt does not examine world knowledge for a certain

\( \hookrightarrow \) sub-dimension (e.g., a simple "a blue sphere"), then **do not generate** a question for that
...
You are a computer vision expert specializing in temporal video analysis and signal processing.

Your expertise is evaluating inter-frame quality, especially distinguishing physically plausible

\( \hookrightarrow \) motion blur
...
1. Artifacts & Degradation:
	...
	- Unnatural blur: Is there blur that cannot be explained by camera motion or fast object motion?

		(e.g., a static object suddenly becomes smeared).
...
\( \hookrightarrow \) recognizable and cause discomfort.

- 2 (Poor): Clearly unsmooth motion with frequent stutters or obvious artifacts;

\( \hookrightarrow \) subject/background often becomes inexplicably blurry.
...
\( \hookrightarrow \) artifacts or abnormal jitter.

#Output Format

Output ONLY a valid JSON string (no Markdown code fences), in the following format:

	"reason": "Provide a detailed analysis of artifacts, smoothness, and scene-aware blur handling
...
\( \hookrightarrow \) like an orthopedic
...
2. Rigid Body Rigidity:
	...
	\( \hookrightarrow \) during movement/camera turns?
...
3. Texture & Semantic Consistency:
	...
	\( \hookrightarrow \) morph over time?
...
\( \hookrightarrow \) (hallucination-like).
...
\( \hookrightarrow \) artifacts introduced
...
\( \hookrightarrow \) coherence of the audio stream.
...
1. Generative Artifacts:
	...
	\( \hookrightarrow \) electronic tone, or phasey/comb-filter effects?
...
2. Temporal Stability:
	...
	\( \hookrightarrow \) foreground audio?
...
\( \hookrightarrow \) mechanical artifacts.
...
\( \hookrightarrow \) liquids, fabric, etc.)
...
\( \hookrightarrow \) sound-source objects and their environment.

Compare visual physical properties with auditory timbre, judge whether they match or cause

\( \hookrightarrow \) perceptual mismatch, then assign an
...
1. Material--Timbre Matching:
	...
	- Core texture: Is the material of the sounding object (e.g., hollow metal pipe vs solid wood
		...
		\( \hookrightarrow \) stick vs shattered glass) reflected correctly in the sound?
...
3. Environmental Acoustics / Reverb:
		...
		\( \hookrightarrow \) open canyon; carpet vs tiles)?
...
\( \hookrightarrow \) immersion-breaking.

- 2 (Poor): Broad material class is roughly right but details are wrong, or audio feels pasted-on

\( \hookrightarrow \) studio-dry sound with no environment.

- 3 (Fair): Generally plausible but lacks fine texture variation; reverb is generic and not

\( \hookrightarrow \) scene-specific.

- 4 (Good): Material timbre is recognizable; footsteps/collisions distinguish surfaces; reverb is

\( \hookrightarrow \) largely appropriate.

- 5 (Perfect): Extremely realistic; convincingly conveys weight, density, and surface texture;

\( \hookrightarrow \) environment acoustics match the scene (cinema-grade foley).

#Output Format

Output ONLY a valid JSON string (no Markdown code fences), in the following format:

	"reason": "Provide a detailed analysis of material recognition, interaction feedback, and

	environmental reverb matching here.",

	"MTC": <an integer from 1 to 5>
